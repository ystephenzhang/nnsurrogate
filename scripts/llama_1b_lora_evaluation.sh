#!/bin/bash
# llama_1b_lora_evaluation.sh
# Evaluation script for Llama-3.2-1B LoRA model
# Model path: /data/expo_output/llama3.2-1b_heat_burgers_zero_shot/v0-20250806-230427/
# Covers Heat_1D, Heat_steady_2D, and Burgers_1D tasks in both zero-shot and iterative settings
# Usage: ./llama_1b_lora_evaluation.sh [GPU_ID]
...
echo "🚀 Starting Llama-3.2-1B LoRA evaluation on GPU $GPU_ID"
...
RESUME_LOG="scripts/llama_1b_lora_evaluation_progress.log"
touch "$RESUME_LOG"
...
# Update .env configuration for Llama-3.2-1B LoRA model
echo "🔧 Updating .env configuration for Llama-3.2-1B LoRA..."

#cat > .env << 'EOF'
## Custom Model Configuration
#custom_code="/home/ubuntu/dev/SimulCost-Bench/custom_model/custom_inference.py"
#model_path="/data/expo_output/llama3.2-1b_heat_burgers_zero_shot/v2-20250809-034208/checkpoint-22/"
#custom_class="Llama3_lora_1B"
#EOF

MODEL_NAME="llama3.2_1b_lora"
MODEL_DISPLAY_NAME="Llama-3.2-1B LoRA"
...
for task in "${!heat_2d_task_modes[@]}"; do
  ...
  for mode in "${task_modes[@]}"; do
    ...
    run_cmd "python inference/langchain_LLM.py -n 50 -p custom_model -m $MODEL_NAME -d 2D_heat_transfer -t $task $mode --resume"
...
# ========================================
echo ""
echo "🎉 LLAMA-3.2-1B LORA EVALUATION COMPLETED SUCCESSFULLY!"
echo ""
...
echo ""
echo "🚀 Llama-3.2-1B LoRA evaluation complete!"