...
def extract_metrics_from_log(log_file_path):
    """Extract success_rate and mean_efficiency from log file summary"""
    try:
        ...
        # Try mean_efficiency first, fallback to mean_hard_efficiency
        efficiency_match = re.search(r'"mean_efficiency":\s*"([^"]+)"', content)
        if not efficiency_match:
            efficiency_match = re.search(r'"mean_hard_efficiency":\s*"([^"]+)"', content)

        if success_match and efficiency_match:
            return {
                'success_rate': float(success_match.group(1)),
                'mean_efficiency': float(efficiency_match.group(1))
            }
        else:
            return None
        ...
        for eval_type in eval_types:
            ...
            for model_name in model_list:
                ...
                for precision in precisions:
                    ...
                    results[model_name][f"{precision}_mean_efficiency"] = None
                ...
                for precision in precisions:
                    ...
                    if log_file.exists():
                        ...
                        if metrics:
                            ...
                            results[model_name][f"{precision}_mean_efficiency"] = metrics['mean_efficiency']
            ...
            if not df.empty:
                ...
                eff_cols = [col for col in df.columns if col.endswith("_mean_efficiency")]

                # Calculate means, ignoring NaN values
                df['mean_success_rate'] = df[sr_cols].mean(axis=1, skipna=True)
                df['mean_efficiency'] = df[eff_cols].mean(axis=1, skipna=True)
                ...
                for precision in precisions:
                    column_order.extend([
                        ...
                        f"{precision}_mean_efficiency"
                    ])
                column_order.extend(['mean_success_rate', 'mean_efficiency'])
...
def main():
    models = ["ab_openai_init_refined_t05",
              ...
              "O_Llama_relaxed_t03",
              "O_Qwen_relaxed_t03",
              "O_Llama_relaxed_final",
              "O_Qwen_relaxed_final",
              "O_claude",
              "G_openai",
              "G_Llama",
              "G_Qwen",
